...
const MealHead = () => {
  ...
  const qq = `Please generate a 10words sentence describing nutrients of the food ${router.query.breakfast} e.g A healthy, protein-rich Nigerian delicacy perfect for breakfast`;
  ...
  const makeRequest = async () => {
    try {
      ...
      const calory = await gemini.post('/gemini-1.5-flash:generateContent', { contents: [{ parts: [{ text: q2 }] }] });
    ...
    <div className="font-satoshi bg-[#8C77EC] p-5 mt-7 rounded-2xl dashboardhead">
      <div className="flex justify-between">
        <div className="flex flex-col gap-5">
          <h1 className=" text-[#D6FBC4] leading-[57px] text-[42px] font-bold">{isClient ? breakfast : '...'}</h1>
          <p className=" text-sm font-light text-[white]">{answer}</p>
          <p className=" font-medium text-sm text-[white]">
            <span className="text-[#FFFFFF] font-jakarta">Calories per serving: </span>{' '}
            <span className="text-[#FFC501]">{isClient ? answer1 : '...'} </span>kcal
          </p>
        </div>
